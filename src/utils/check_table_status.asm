...
check_table_status:
	...
	addi $t0, $t0, 4 #as status is 4 bytes deep in table array, we need to add it to get the correct address
	lw $t1, TABLE_STATUS($t0) #reads the address of the specific table and stores the contents in $t1
	beq $t1, $zero, table_available #If the status of the table is set to 0, then it is available.
	j table_unavailable #if the status of the table is NOT 0, then it's not available.
